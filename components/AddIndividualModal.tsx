...
const AddIndividualModal: React.FC<AddIndividualModalProps> = ({ currentUser, onClose, onSave }) => {
  ...
  const [searchResults, setSearchResults] = useState<{id: string, nome: string, alcunha: string}[]>([]);
  const [selectedRelType, setSelectedRelType] = useState<'COMPARSA' | 'PARENTE' | 'VIZINHO'>('COMPARSA');
  ...
  const addRelationship = (rel: {id: string, nome: string}) => {
    if (relationships.some(r => r.relacionado_id === rel.id)) return;
    setRelationships([...relationships, { relacionado_id: rel.id, nome: rel.nome, tipo: selectedRelType }]);
    setSearchTerm('');
    setSearchResults([]);
  };

  const removeRelationship = (id: string) => {
    setRelationships(relationships.filter(r => r.relacionado_id !== id));
  };
    ...
    try {
      ...
      if (indData) {
        ...
        if (relationships.length > 0) {
          await supabase.from('individuos_relacionamentos').insert(relationships.map(r => ({
            individuo_id: indData.id, relacionado_id: r.relacionado_id, tipo: r.tipo, created_by: currentUser?.nome
          })));
        }
      }
      onSave();
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md overflow-y-auto">
      <div className="bg-slate-800 border border-slate-700 w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden my-auto animate-in fade-in zoom-in duration-300">
        ...
        <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2">
              ...
              <input type="text" className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-yellow-600 outline-none" value={formData.nome} onChange={e => setFormData({...formData, nome: e.target.value})} required />
            </div>
            <div>
              <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Alcunha</label>
              <input type="text" className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-yellow-600 outline-none" value={formData.alcunha} onChange={e => setFormData({...formData, alcunha: e.target.value})} />
            </div>
            <div>
              <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Data de Nascimento</label>
              <input type="date" className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-yellow-600 outline-none" value={formData.data_nascimento} onChange={e => setFormData({...formData, data_nascimento: e.target.value})} />
            </div>
            <div>
              <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">CPF</label>
              <input type="text" className={`w-full bg-slate-900 border ${cpfError ? 'border-red-500' : 'border-slate-700'} rounded-xl px-4 py-3 text-white outline-none`} value={formData.documento} onChange={handleCpfChange} maxLength={14} />
            </div>
            <div>
              <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Facção</label>
              <select className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-yellow-600 outline-none appearance-none" value={formData.faccao} onChange={e => setFormData({...formData, faccao: e.target.value})}>
          ...
          <div className="space-y-4 pt-4 border-t border-slate-700">
            <div className="flex items-center justify-between mb-2">
               <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Outros Anexos (Documentos / Docs Diversos)</h4>
          ...
          <div className="space-y-4 pt-4 border-t border-slate-700">
            <div className="flex items-center justify-between">
              <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest">Fotos de Perfil</label>
              <button type="button" onClick={() => fileInputRef.current?.click()} className="text-yellow-600 hover:text-yellow-500 text-[10px] font-black uppercase flex items-center">
                <i className="fas fa-camera mr-2"></i> Bater / Subir Foto
              </button>
              <input type="file" ref={fileInputRef} onChange={handlePhotoChange} className="hidden" multiple accept="image/*" />
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-4">
              {photos.map((photo) => (
                <div key={photo.id} className={`relative aspect-square rounded-xl border-2 overflow-hidden ${photo.isPrincipal ? 'border-yellow-600' : 'border-slate-700'}`}>
                  <img src={photo.data} className="w-full h-full object-cover" />
                  {photo.isPrincipal && <div className="absolute top-1 left-1 bg-yellow-600 text-[8px] font-black px-1.5 rounded">CAPA</div>}
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex gap-4 pt-4 border-t border-slate-700">
            <button type="button" onClick={onClose} className="flex-1 bg-slate-700 text-white font-black py-4 rounded-xl uppercase text-xs">Cancelar</button>
            <button type="submit" disabled={isSaving} className="flex-1 bg-yellow-600 text-white font-black py-4 rounded-xl uppercase text-xs shadow-lg shadow-yellow-600/20">
              {isSaving ? <i className="fas fa-spinner fa-spin"></i> : 'Finalizar Cadastro'}
            </button>
          </div>
        </form>
      </div>
